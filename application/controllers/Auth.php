<?php
    ...
    class Auth extends CI_Controller{
        ...
        // function pass(){
        //     $data = password_hash("admin", PASSWORD_BCRYPT);
        //     echo $data;
        // }
        ...
        function checkLogin(){
            ...
            if($this->model_auth->checkUser($email)){
                $dataUser = $this->model_auth->getDataUser(["email" => $email]);
                
                if(!array_key_exists($dataUser["tipe"], $this->model_users->listType)){
                    $data["result"] = false;
                    $data["msg"] = "Pengguna tidak memilki hak akses";
                }
                else{
        ...
        function checkLevel(){
            if($this->session->userdata("tipe") == "Super"){ return true; }
            else{ return false; }
        }
        ...
        function newUserPass(){
            if($this->checkLevel()){
                ...
                if($this->model_auth->checkPegawai($kdepeg)){
                    ...
                    if($this->model_auth->checkUser($email)){
                        $data['result'] = false;
                        $data['msg'] = "email telah digunakan. <br/> Silahkan mencoba email lain.";
                    }
                    else{
                        $password = password_hash($this->input->post("password"), PASSWORD_BCRYPT);
                        $kdeaplikasi = $this->input->post("kdeaplikasi");
                        $tipe = $this->input->post("user_level");
