								...
								<div class="kt-portlet__head kt-portlet__head--lg">
									<div class="kt-portlet__head-label">
										...
										<h3 class="kt-portlet__head-title">
											<?php
												if($edit) { echo "Edit Transaksi Tupoksi ". $tupoksi['kdepeg'];}
												else { echo "Transaksi Tupoksi Baru";}
											?>
										</h3>
									</div>
								</div>
								<form action="#" method="post" enctype="multipart/form-data" id="tupoksiForm">
									<div class="kt-portlet__body">
										<div class="row">
													...
													<select class="form-control m-select2" id="periodeForm" name="periode" required>
														...
														<?php
															foreach($periode as $row){
																if($edit && $row->periode == $tupoksi['periode']){
											...
											<div class="col-md-6 col-sm-12 col-lg-6">
												<div class="form-group">
													<label>Bulan</label>
													<select class="form-control m-select2" id="bulanForm" name="bulan" required>
														<option value="" selected disabled>Pilih Bulan</option>
														<?php
															for($i = 1; $i <= 6; $i++){
																if($edit && $i == $tupoksi['bulan']){
													...
													<select class="form-control m-select2" id="pegawaiForm" name="pegawai" required>
														...
														<?php
															foreach($pegawai as $row){
																if($edit && $row->kdepeg == $tupoksi['kdepeg']){
													...
													<select class="form-control m-select2" id="peranForm" name="peran" required>
														...
														<?php
															foreach($peran as $row){
                                                                if($row->kdeaktivitas[0] != 1){
                                                                    continue;
                                                                }

																if($edit && $row->kdeperan == $tupoksi['kdeperan']){
										...
										<div class="row">
													...
													<select class="form-control m-select2" id="dokumenForm" name="dokumen" required>
														...
														<?php
															foreach($dokumen as $row){
																if($edit && $row->udi == $tupoksi['udi_trdokumen']){
                                            ...
                                            <div class="col-md-6 col-sm-12 col-lg-6">
												<div class="form-group">
													<label>Capaian</label>
													<input type="number" class="form-control" placeholder="Enter Capaian" min="1" name="capaian" id="capaianForm" required <?php if($edit){ echo 'value="'.$tupoksi['capaian'].'"';} ?> step=".01">
												</div>
											</div>
                                            <div class="col-md-6 col-sm-12 col-lg-6">
												<div class="form-group">
													<label>Poin Aktivitas</label>
													<input type="number" class="form-control" placeholder="Enter Point Aktivitas" name="poin_aktivitas" id="pointAktivitasForm" required <?php if($edit){ echo 'value="'.$tupoksi['poinaktivitas'].'"';} ?> step=".01">
												</div>
											</div>
                                        </div>
                                        <div class="form-group row">
                                            <label class="col-md-2 col-sm-12 col-form-label">Status Tupoksi :</label>
                                            <div class="col-md-10 col-sm-12">
                                                <div class="kt-radio-inline">
                                                    <label class="kt-radio kt-radio--success">
                                                        <input type="radio" name="status_tupoksi" value="Aktif" <?php if($edit && $tupoksi['stsrek'] == "Aktif"){echo 'checked';} else if(!$edit){echo 'checked';} ?>> Aktif
                                                        <span></span>
                                                    </label>
                                                    <label class="kt-radio kt-radio--danger">
                                                        <input type="radio" name="status_tupoksi" value="Batal" <?php if($edit && $tupoksi['stsrek'] == "Batal"){echo 'checked';} ?>> Batal
                                                        <span></span>
                                                    </label>
                                                </div>
                                            </div>
